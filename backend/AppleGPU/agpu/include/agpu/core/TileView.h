// TileView - a rectangular tile resident somewhere addressable. Whoever
// reserves space for a tile calls cosizeElems() on the same object the emitter
// addresses through.
...
struct Swizzle {
  ...
  int phaseDim = 0;
  int groupDim = 1;
  // How wide the permutation reaches; a wider row holds several tiles of it.
  int64_t groupExtent = 0;
  // Distance to the next tile. Zero leaves the tiles inline.
  ...
  // the parent's rather than shrinking to its own width.
  int64_t spanOver(int64_t viewExtent) const {
    return groupExtent > 0 ? groupExtent : viewExtent;
  }

  // The XOR must land inside the span, so the phase is clamped to the number
  // of groups the span holds.
...
// gain `pad` more. Each rule measures on the unpadded offset, which keeps the
// map monotonic.
struct Padding {
  struct Rule {
    int64_t interval = 0;
...
// Extents and strides are in elements, innermost dimension last.
class TileView {
public:
  ...
  // in Emit.h (AST nodes) share one loop, and so both spellings permute alike.
  ...
  // Coordinates stay the tensor's; the origin absorbs the subtraction and goes
  ...
  // A pool reservation must be at least this large. A swizzle moves the
  // highest offset off the last coordinate, so the row span bounds it, not the
  // corner.
