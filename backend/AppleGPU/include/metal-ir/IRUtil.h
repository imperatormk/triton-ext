...
inline void
collectTGGlobals(llvm::Module &M,
...
inline void
collectTGByteGlobals(llvm::Module &M,
                     llvm::SmallVectorImpl<llvm::GlobalVariable *> &out) {
  for (auto &GV : M.globals()) {
    if (GV.getAddressSpace() != AS::Threadgroup)
      continue;
...
inline void
collectTGTypedGlobals(llvm::Module &M,
                      llvm::SmallVectorImpl<llvm::GlobalVariable *> &out) {
  for (auto &GV : M.globals()) {
    if (GV.getAddressSpace() != AS::Threadgroup)
      continue;
...
inline llvm::Type *inferElementType(llvm::Value *V) {
  for (auto *U : V->users()) {
    ...
    if (llvm::isa<llvm::GetElementPtrInst>(U) ||
        llvm::isa<llvm::GEPOperator>(U) || llvm::isa<llvm::BitCastInst>(U))
...
inline bool scalarizeWideVecStores(llvm::Value *V, llvm::Type *I32Ty) {
  ...
  for (auto *SI : wideStores) {
    auto *VT =
        llvm::cast<llvm::FixedVectorType>(SI->getValueOperand()->getType());
  ...
  for (auto &BB : F) {
    for (auto it = BB.begin(); it != BB.end();) {
      ...
      if (auto *EE = llvm::dyn_cast<llvm::ExtractElementInst>(&I)) {
        if (auto *IE = llvm::dyn_cast<llvm::InsertElementInst>(
                EE->getVectorOperand())) {
