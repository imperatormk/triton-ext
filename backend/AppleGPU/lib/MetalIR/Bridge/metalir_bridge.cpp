// C bridge: metalir_compile(ir, outLen, errbuf, errlen) -> malloc'd metallib
// bytes (freed with metalir_free).
...
__attribute__((visibility("default"))) void *metalir_compile(const char *llText,
                                                             uint64_t *outLen,
                                                             char *errbuf,
                                                             int errlen) {
  LLVMContext Ctx;
  ...
  // Leave triple/datalayout unset: the metallib header carries the platform
  // bytes, and setting them in the bitcode can trigger materializeAll failures
  // on some Metal GPU JIT versions.
...
metalir_tg_memory_bytes(const char *llText) {
  LLVMContext Ctx;
  SMDiagnostic Err;
  auto Buf = MemoryBuffer::getMemBuffer(llText, "triton_kernel");
  auto M = parseIR(*Buf, Err, Ctx);
...
__attribute__((visibility("default"))) void metalir_free(void *ptr) {
  free(ptr);
}
