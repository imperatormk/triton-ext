...
PreservedAnalyses AIRSystemValuesPass::run(Module &M,
  ...
  for (auto *FPtr : funcs) {
    ...
    // We need to create a new function with extra params and replace the old
    // one
    ...
    for (auto &F : M) {
      ...
      for (unsigned i = 0; i < FTy->getNumParams(); i++) {
        ...
        const char *accessMode =
            (addrSpace == 2) ? air::kMDRead : air::kMDReadWrite;
        paramNodes.push_back(MDNode::get(
            ...
            {ConstantAsMetadata::get(ConstantInt::get(I32, i)),
             ...
             MDString::get(Ctx, "float"), MDString::get(Ctx, air::kMDArgName),
      ...
      for (auto &Arg : F.args()) {
        ...
        // System value: no arg_type_size/align.
        ...
        paramNodes.push_back(MDNode::get(
            ...
            {ConstantAsMetadata::get(ConstantInt::get(I32, Arg.getArgNo())),
             ...
             MDString::get(Ctx, air::kMDArgTypeName),
             MDString::get(Ctx, typeName), MDString::get(Ctx, air::kMDArgName),
