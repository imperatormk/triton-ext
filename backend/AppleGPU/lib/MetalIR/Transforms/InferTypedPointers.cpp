...
//
// Includes MMA-specific pointer-type overrides.
...
PreservedAnalyses InferTypedPointersPass::run(Module &M,
    ...
    for (auto &F : M) {
      for (auto &BB : F) {
        for (auto &I : BB) {
          ...
          if (currentPointee == expectedPointee)
            continue;
  ...
  // Phase 5: MMA-specific overrides.
  if (constraints.hasMMA) {
  ...
  //   6. Set wait_simdgroup_events pointer param to event_t
  {
    ...
    if (hasAsyncCopy) {
      ...
      StructType *eventTy =
          StructType::getTypeByName(M.getContext(), "event_t");
      if (!eventTy)
  ...
  // Phase 7: Fix up ptr-to-ptr bitcasts that serve as typed pointer
  // transitions.
  //
