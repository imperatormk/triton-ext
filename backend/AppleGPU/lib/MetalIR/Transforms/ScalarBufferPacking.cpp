  ...
  for (auto *FPtr : funcs) {
    ...
    SmallDenseMap<unsigned, Type *, 16> descriptorTypeForParam;
    {
      ...
      while (i < F.arg_size()) {
        ...
        bool isDevicePtr =
            T->isPointerTy() && T->getPointerAddressSpace() == AS::Device;
        if (isDevicePtr) {
          ...
          while (j < F.arg_size()) {
            ...
            if (!Tj->isPointerTy() ||
                Tj->getPointerAddressSpace() != AS::Constant)
              break;
            j++;
          }
    ...
    // Metal v1 bitcode rejects load type != pointee type (e.g. load i32 from
    // i8*).
    ...
    // scalar buffer must be the LAST device buffer param (before system
    // values).
    ...
    for (unsigned j = 0; j < scalarParams.size(); j++) {
      ...
      if (sp.scalarType == bufElemTy) {
        ...
        if (sp.scalarType->isPointerTy()) {
          // Pointer: load as two i32 words (lo, hi), combine to i64, then
          // inttoptr
          auto *rawLoad =
          ...
          if (sp.scalarType->isIntegerTy()) {
            ...
            if (targetBits > 32) {
              ...
              // Can't load i64 directly from float* (Metal typed pointer
              // crash).
            ...
            // i64 scalars: would need two float loads — for now just load
            // directly
