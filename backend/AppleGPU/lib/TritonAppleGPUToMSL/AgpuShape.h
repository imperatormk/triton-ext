// AgpuShape - a tensor's shape as counts and views.
...
// Every axis of the tensor, full: what a whole-tensor staging plans against.
...
// Each source element's flat index in the result's numbering. `order[d]` is
// the source axis result axis `d` comes from, the `tt.trans` convention.
