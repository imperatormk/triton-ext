...
void MSLEmitter::restoreBodyState(const BodyWalkState &s) {
  static_cast<BodyWalkState &>(*this) = s;
...
LogicalResult MSLEmitter::emitFunc(tt::FuncOp func) {
  ...
  BodyWalkState preBody = *this;
  msl::Block body = walkBody();
  if (emitFailed)
    return failure();
  FuncSize built = measureFunc(body);
  ShrinkPlan plan = planShrink(built);
  debugBudget(func.getName(), "built", built, plan);
  if (plan.rollKSteps) {
    restoreBodyState(preBody);
    fnRollKSteps = true;
    body = walkBody();
    if (emitFailed)
      return failure();
    debugBudget(func.getName(), "rolled", measureFunc(body), plan);
  }
  ...
  shrink(fn, ctx, plan);
  debugBudget(func.getName(), "final", measureFunc(fn->body), plan);
