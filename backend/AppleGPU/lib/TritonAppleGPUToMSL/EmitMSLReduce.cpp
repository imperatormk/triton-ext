...
// cross-lane warp-carry sequences use the simd_shuffle family via
// builtin::simd; the map_elementwise unstructured region is modelled as a real
...
msl::Expr *MSLEmitter::astReduceLaneShuffle(StringRef sc, StringRef acc,
                                            unsigned m) {
  return astShuffleExpr(bsimd::ShuffleXor, sc, acc, std::to_string(m) + "u");
}
...
msl::Expr *MSLEmitter::astReduceWarpBase(unsigned warpMask) {
  msl::Expr *masked = ctx.paren(
      ctx.binary(B::And, ctx.var(warpId), ctx.lit(std::to_string(~warpMask))));
  return ctx.paren(ctx.binary(B::Add, ctx.binary(B::Mul, masked, ctx.lit("32")),
                              ctx.var(laneId)));
}
...
msl::Stmt *MSLEmitter::astReduceWarpLoad(StringRef sc, StringRef dst,
                                         ...
                                         int off) {
  msl::Expr *idx =
      off == 0 ? base : ctx.binary(B::Add, base, ctx.lit(std::to_string(off)));
...
msl::Expr *MSLEmitter::astScanLaneShuffle(bool rev, StringRef sc, StringRef val,
                                          unsigned delta) {
...
msl::Stmt *MSLEmitter::astScanGuardedUpdate(StringRef dst, msl::Expr *guard,
  ...
  return ctx.assignStmt(
      ctx.var(dst), ctx.paren(ctx.ternary(guard, ctx.var(out), ctx.var(dst))));
}
...
msl::Stmt *MSLEmitter::astWarpCarryMyPart(StringRef myPart,
  ...
  auto term = [&](int b, int r) -> msl::Expr * {
    ...
    return ctx.paren(
        ctx.paren(ctx.binary(B::Shl, bit, ctx.lit(std::to_string(r)))));
  };
