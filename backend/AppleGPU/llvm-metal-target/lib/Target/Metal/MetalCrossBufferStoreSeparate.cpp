...
// predicate is always taken for in-bounds threads (identical semantics) but is a
// real conditional branch, so the JIT can no longer place the cross-buffer pair
// in one straight-line run -> it cannot address-coalesce them. This is precisely
// the transform that distinguishes the passing -O0 IR from the failing -O1 IR.
//
...
//   * an offset group is a TRIGGER when >=2 of its stores reach DIFFERENT buffer
...
// so single-output kernels (no two device-output buffers at the same offset) are
// a guaranteed byte-identical no-op.
//
//===----------------------------------------------------------------------===//
  ...
  for (Value *Off : Cands) {
    ...
    for (User *U : Off->users()) {
      ...
      if (IC->getParent() == &IC->getFunction()->getEntryBlock())
        return IC;
      if (IC->getParent() == InsertPt->getParent() &&
          IC->comesBefore(InsertPt))
        return IC;
    }
  }
  return nullptr;
...
// GEPs they depend on. Non-conflicting stores stay in `cont` (unguarded, exactly
// as before), so a store with a DIFFERENT predicate/offset is never pulled under
// this guard. A real conditional branch on the in-bounds predicate is what makes
// the AGX JIT stop treating the warp-0 lane stores as unconditionally executed,
// so it no longer address-coalesces the cross-buffer pair. `Guard` is an i1 true
// for in-bounds threads; semantics are preserved exactly (out-of-bounds threads
// never stored anyway). Returns the continuation block for re-scanning.
static BasicBlock *guardStoreGroup(ArrayRef<StoreInst *> Group, Value *Guard) {
  ...
  // Anchor the hoist at the EARLIEST store in program order (the safety check in
  // the caller guarantees every store's value is defined before this point).
  ...
  // fresh empty StoreBB between Pre and Tail to hold exactly the group's stores.
  BasicBlock *Tail = Pre->splitBasicBlock(At, Pre->getName() + ".cbssep.tail");
  BasicBlock *StoreBB =
      BasicBlock::Create(Pre->getContext(), Pre->getName() + ".cbssep.st",
                         Pre->getParent(), Tail);
  ...
  for (StoreInst *SI : Group) {
    // Collect the pure operand chain (pointer + value) living in Tail by a small
    // DFS, then move it in dependency order (operands before users). Multi-use
    // instructions are pulled too — the caller guaranteed every such value's
    // users are stores in this group, which all move into StoreBB, so the def
    // still precedes every use. `Moved` dedups shared glue.
    ...
    std::function<void(Value *)> pull = [&](Value *V) {
      ...
      if (!I || I->getParent() != Tail || I->mayHaveSideEffects() ||
        ...
        return;
      // Only relocate a (possibly multi-use) instruction when EVERY user is also
      // being hoisted: a store in this group, or another instruction already
      // marked to move. Otherwise a user left behind in Tail would see its def
      // sink into the predicated StoreBB and no longer dominate it.
      for (User *U : I->users()) {
        auto *UI = dyn_cast<Instruction>(U);
        bool UserHoisted =
            (UI && (Moved.count(UI) || Seen.count(UI))) ||
            (isa<StoreInst>(U) &&
...
static bool separateInFunction(Function &F) {
  ...
  while (!Worklist.empty()) {
    ...
    // forced under the wrong guard. Process one group per scan, then re-scan the
    // continuation (which now holds the remaining stores) for the next group.
    ...
    // Pick one offset group that is genuinely cross-buffer, has a real mask, and
    // is safe to hoist. Hoisting moves the whole group to the position of its
    // FIRST store; for the result to dominate-check, every store's stored VALUE
    // (and pointer) must already be available there. A value computed AFTER the
    // first store (e.g. a `fptrunc` the mid-end sank below the store run) would
    // become a use-before-def -> the AIR writer emits an invalid forward record.
    // Such a group is skipped (left un-separated) rather than miscompiled.
    ...
    for (auto &KV : ByOffset) {
      ...
      for (StoreInst *SI : KV.second)
        if (SI->comesBefore(FirstSI))
          FirstSI = SI;
      // guardStoreGroup relocates each store along with the pure single-use glue
      // feeding its pointer and value (e.g. a vectorized result's
      // `extractelement`). That keeps dominance EXCEPT when a store's value is
      // defined after FirstSI AND cannot be moved with it — i.e. it is impure or
      // shared (multi-use), so it must stay where it is and would then be used
      // before its def. Detect that case and skip the group (leave it
      // un-separated) rather than emit an invalid forward reference. A value
      // defined before FirstSI, or a movable pure single-use def, is always safe.
      // A store's value defined after FirstSI is fine as long as it is PURE and
      // every one of its users is a store in THIS group — then it (and they) all
      // move together into StoreBB and the def still precedes every use. A
      // duplicated output (out1==out4) makes the shared `extractelement`
      // multi-use, which is safe precisely because all those uses are hoisted.
      // An impure value, or one consumed by something outside the group (a later
      // non-store use, or a foreign store), cannot be relocated -> skip.
