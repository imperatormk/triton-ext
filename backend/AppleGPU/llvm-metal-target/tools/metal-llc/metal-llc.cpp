//===- metal-llc.cpp ----------------------------------------------------===//
//
// Driver that mirrors the subset of `llc` this backend invokes:
//
//   metal-llc -mtriple=air -filetype=obj -o OUT IN
//
// Reads textual LLVM IR (or `-` for stdin), runs the Metal codegen pipeline,
// and emits a metallib byte stream. The backend points METAL_LLC_PATH at this
// binary to lower LLVM IR to .metallib.
//===---------------------------------------------------------------------===//
