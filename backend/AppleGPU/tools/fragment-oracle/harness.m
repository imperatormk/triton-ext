...
int main(int argc, char **argv) {
  @autoreleasepool {
    const char *libpath =
        argc > 1 ? argv[1]
                 : "/Users/zimski/projects/oss/triton-ext/backend/AppleGPU/"
                   "tools/fragment-oracle/oracle.metallib";
    id<MTLDevice> dev = MTLCreateSystemDefaultDevice();
    id<MTLCommandQueue> q = [dev newCommandQueue];
    NSError *err = nil;
    id<MTLLibrary> lib = [dev
        newLibraryWithURL:[NSURL
                              fileURLWithPath:[NSString
                                                  stringWithUTF8String:libpath]]
                    error:&err];
    if (!lib) {
      NSLog(@"lib err %@", err);
      return 1;
    ...
    id<MTLBuffer> bIn = [dev newBufferWithLength:64 * 4
                                         options:MTLResourceStorageModeShared];
    id<MTLBuffer> bLoad =
        [dev newBufferWithLength:64 * 4 options:MTLResourceStorageModeShared];
    id<MTLBuffer> bCol = [dev newBufferWithLength:64 * 4
                                          options:MTLResourceStorageModeShared];
    id<MTLBuffer> bRow = [dev newBufferWithLength:64 * 4
                                          options:MTLResourceStorageModeShared];
    id<MTLBuffer> bExp = [dev newBufferWithLength:64 * 4
                                          options:MTLResourceStorageModeShared];
    id<MTLBuffer> bEpi = [dev newBufferWithLength:64 * 4
                                          options:MTLResourceStorageModeShared];
    float *pat = bIn.contents;
    for (uint r = 0; r < 8; r++)
      for (uint c = 0; c < 8; c++)
        pat[r * 8 + c] = (float)(r * 8 + c + 1);
    ...
    [enc setBuffer:bLoad offset:0 atIndex:1];
    [enc setBuffer:bCol offset:0 atIndex:2];
    [enc setBuffer:bRow offset:0 atIndex:3];
    [enc setBuffer:bExp offset:0 atIndex:4];
    [enc setBytes:&mutate length:4 atIndex:5];
    ...
    printf("  [neg-control] %s (swapped masks: col %d/64 row %d/64 epi %d/64 "
           "wrong -> expected nonzero)\n",
