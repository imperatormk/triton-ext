...
kernel void frag_oracle(
    device const float *inPat [[buffer(0)]], // 8x8 row-major
    device float *loadOut [[buffer(1)]],     // [lane*2+reg]
    device float *colOut [[buffer(2)]], device float *rowOut [[buffer(3)]],
  ...
  // col-replicate, but the source value is the slice element for this row.
  {
    uint src = lane & ~colClr;
    float sliceVal =
        simd_shuffle(reg[0], src); // reg[0] at src holds in[row][0]
  ...
  // mutate!=0 corrupts srcLane (drop L1) so the restripe MUST fail.
  {
    ...
    for (uint r = 0; r < 2; ++r) {
      ...
      if (mutate)
        src &= ~0x2u; // drop L1 -> wrong source lane
